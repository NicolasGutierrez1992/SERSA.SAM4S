...
export class NotificacionesService {
  ...
  async notificarLimiteAlcanzado(
    userId: number,
    limite: number,
  ): Promise<void> {
  ...
  async notificarLimite80Porciento(
    userId: number,
    actual: number,
    limite: number,
  ): Promise<void> {
  ...
  async notificarDescargaCompletada(
    userId: number,
  ...
  async findAll(queryDto: QueryNotificacionesDto = {}) {
    const { user_id, tipo, estado_envio, page = 10, limit = 100 } = queryDto;
    ...
    if (user_id !== undefined) {
      queryBuilder.andWhere('notificacion.destinatario_id = :user_id', {
        user_id,
      });
    }
    ...
    if (estado_envio !== undefined) {
      queryBuilder.andWhere('notificacion.estado_envio = :estado_envio', {
        estado_envio,
      });
    }
