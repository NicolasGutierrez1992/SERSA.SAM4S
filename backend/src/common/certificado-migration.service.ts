...
export class CertificadoMigrationService {
  ...
  async migrarSiEsNecesario(
    ...
    try {
      this.logger.log(
        'Verificando si es necesaria migración de certificado...',
      );
      ...
      if (existeEnBd) {
        this.logger.log(
          'Certificado maestro ya existe en BD, no es necesaria migración',
        );
        return;
      }
      ...
      if (!fs.existsSync(certPath)) {
        this.logger.warn(
          `Archivo de certificado no encontrado: ${certPath}. Migración no realizada.`,
        );
        return;
      }
      ...
      this.logger.log(
        'Certificado migrado exitosamente a base de datos',
        resultado,
      );
    } catch (error) {
